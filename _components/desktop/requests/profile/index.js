(function () {
    ...
    function initPlugin() {
        ...
        F.prototype.init_table = function () {
            ...
            var DSParams = ADVT.DataSource.SimplePostParams({'url': '/admin/Profiles/API?action=list', method: 'post'}, ADVT.DataSource.Extractor.MixExtractor({}));
        ...
        F.prototype.load_editor = function () {
            this.showLoader();
            Y.load('data_editor.request.profile')
        ...
        F.prototype.onCommandRemove = function (t) {
            ...
            EFO.simple_confirm()
                    ...
                    .set_text("Удалить этот вид деятельности?<br><b style=\"color:crimson;font-size:.9em\">Это действие нельзя отменить<b>")
                    .set_callback(this, function (confirm, index) {
                        if (U.IntMoreOr(index, 0, 0) === 2) {
                            this.table.reloadFromUrl("/admin/Profile/API?action=remove&id_to_remove=" + id);
                        }
                    })
                    .set_style("blue")
                    .set_icon("baloon?")

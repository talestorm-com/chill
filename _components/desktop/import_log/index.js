(function () {
    ...
    function initPlugin() {
        ...
        F.prototype.onInit = function () {
            ...
            this.init_table();
            this.mark_logs_reden();
            return this;
        };

        F.prototype.mark_logs_reden = function () {
            var n = jQuery('.AdminImportMenuNotificator');
            if (n && n.length) {
                var s = U.IntMoreOr(n.data('log_id'), 0, 0);
                if (s) {
                    n.removeClass('').addClass('AdminImportMenuNotificator');
                    localStorage.setItem('import_log_notificator_id', s);
                }
            }
        }
        ...
        F.prototype.table_def = function () {
            return {
                ...
                columns: [
                    ...
                    {id: "updated", key: "updated", property: "updated", "text": "Дата", filter: "Date", sort: true},
                    {id: "state", key: "state", property: "state", "text": "Статус", filter: false, sort: false},
                    {id: "rm", key: "rm", property: "rm", "text": "x", filter: false, sort: false}
                ]
            };
        };
        ...
        F.prototype.init_table = function () {
            ...
            this.table.addRenderer('translate_state', function () {
                var key = ["S", this.state].join('');
                return U.NEString(STATES[key], key);
            });

            var DSParams = ADVT.DataSource.SimplePostParams({'url': '/admin/Import/API?action=list', method: 'post'}, ADVT.DataSource.Extractor.MixExtractor({}));
        ...
        F.prototype.on_editor_ready = function (x) {
            return x.show().load(this._id_to_edit);
            return this;
        };
        ...
        F.prototype.onCommandRemove = function (t) {
            ...
            EFO.simple_confirm()
                    ...
                    .set_text("Удалить этот лог?<br><b style=\"color:crimson;font-size:.9em\">Это действие нельзя отменить<b>")
                    .set_callback(this, function (confirm, index) {
                        if (U.IntMoreOr(index, 0, 0) === 2) {
                            this.table.reloadFromUrl("/admin/Import/API?action=remove&id_to_remove=" + id);
                        }
                    })
                    .set_style("blue")
                    .set_icon("baloon?")
        ...
        F.prototype.onCommandClear_old = function(){
            EFO.simple_confirm()
                    .set_title("Подтверждение")
                    .set_text("Удалить логи старше месяца?<br><b style=\"color:crimson;font-size:.9em\">Это действие нельзя отменить<b>")
                    .set_callback(this, function (confirm, index) {
                        if (U.IntMoreOr(index, 0, 0) === 2) {
                            this.table.reloadFromUrl("/admin/Import/API?action=clear_old");
                        }
                    })
                    .set_style("blue")
                    .set_icon("baloon?")

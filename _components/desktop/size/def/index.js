(function () {
    ...
    function initPlugin() {
        ...
        F.prototype.table_def = function () {
            return {
                ...
                columns: [
                    ...
                    {id: "short_name", key: "short_name", property: "short_name", "text": "Сокращенное", filter: "String", sort: true},
                    {id: "visible", key: "visible", property: "visible", "text": "Видимый", filter: "Bool", sort: true},
                    {id: "info", key: "info", property: "info", "text": "Инфо", filter: false, sort: false},
        ...
        F.prototype.init_table = function () {
            ...
            var DSParams = ADVT.DataSource.SimplePostParams({'url': '/admin/SizeVoc/API?action=get_size_systems', method: 'post'}, ADVT.DataSource.Extractor.MixExtractor({}));
        ...
        F.prototype.load_editor = function () {
            this.showLoader();
            Y.load('data_editor.size.sizedef')
        ...
        F.prototype.on_editor_ready = function (x) {
            return x.show().load(this.__to_edit).setCallback(this, this.reload);
            return this;
        };
        ...
        F.prototype.onCommandRemove = function (t) {
            ...
            EFO.simple_confirm()
                    ...
                    .set_text("Удалить эту таблицу размеров?<br><b style=\"color:gray;font-size:.9em\">Это действие удалит все размеры данной таблицы из всех товаров<b>")
                    .set_callback(this, function (confirm, index) {
                        if (U.IntMoreOr(index, 0, 0) === 2) {
                            this.table.reloadFromUrl("/admin/SizeVoc/API?action=remove_size_table&id_to_remove=" + user_id);
                        }
                    })
                    .set_style("blue")
                    .set_icon("baloon?")

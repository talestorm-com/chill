(function () {
    ...
    function initPlugin() {
        ...
        F.prototype.on_data_success = function (d) {            
            var items = [];
            var c = U.safeObject(d.info);
            for(var k in c){
                if(c.hasOwnProperty(k)){
                    var item = {name:k,value:null};
                    if(U.NEString(c[k],null)){
                        item.value=U.NEString(c[k],null);
                        items.push(item);
                    }else if(U.isObject(c[k])|| U.isArray(c[k])){
                        item.value = JSON.stringify(c[k]);
                        items.push(item);
                    }
        ...
        F.prototype.render = function () {
            this.getRole('list').html(Mustache.render(EFO.TemplateManager().get('item', MC), this));
            return this;
        }

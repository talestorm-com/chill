    ...
    function initPlugin() {
        ...
        F.prototype.on_file_changed = function () {
            ...
            if (this.getRole('file').get(0).files.length) {
                ...
                reader.readAsDataURL(this.getRole('file').get(0).files[0]);
            }
        };

        F.prototype.on_new_file = function (e) {
            this.getRole('image').attr('src', e.target.result);
            return this;
        };
        ...
        //</editor-fold>     
        //
        F.prototype._set_field_image_name = function (d, fi) {            
            var id = U.IntMoreOr(d.id, 0, null);
            if (id) {
                this.getField('image_name').attr('src', ["/media/SMILE/", id, '/smile.SW_100H_100CF_1B_ffffff.png'].join(''));
            } else {
                this.getField('image_name').attr('src', '');
            }
        };
        F.prototype._get_field_image_name = function (d, fi) {
            return 'smile';
        };
        ...
        F.prototype.save = function (keep_open) {
            ...
            if (this.getRole('file').get(0).files.length) {
                form_data.append('image', this.getRole('file').get(0).files[0]);
            }

            this.showLoader();
            form_data.append('action', 'put');
            form_data.append('data', JSON.stringify(data));

            jQuery.ajax({url: '/admin/EmojiList/API', data: form_data, processData: false,
                contentType: false, dataType: 'json', method: 'POST'
            })

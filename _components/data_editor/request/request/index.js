(function () {
    ...
    function initPlugin() {
        ...
        F.prototype.on_meta_success = function (sl, d) {
            sl = U.safeArray(sl);
            d.status_id = U.IntMoreOr(d.status_id, 0, null);
            var selected_status = null;
            for (var i = 0; i < sl.length; i++) {
                if (d.status_id === U.IntMoreOr(sl[i].id, 0, void(0))) {
                    selected_status = d.status_id;
                    break;
                }
            }
            if (!selected_status) {
                sl = [{id: d.status_id, name: d.status_name}].concat(sl);
            }
            this.statuses = sl;
            this.getField('status').html(Mustache.render(EFO.TemplateManager().get('status', MC), this));
            return this;
        };
        ...
        F.prototype.on_load_responce = function (d) {
            if (U.isObject(d)) {
                if (d.status === "ok") {
                    this.on_meta_success(d.statuses, d.data);
        ...
        F.prototype._set_field_status = function (data, field) {
            field.val(U.IntMoreOr(data.status_id));
            return this;
        };

        F.prototype._set_field_position_cost = function (data, field) {
            field.val(EFO.Checks.formatPriceNSD(U.FloatMoreEqOr(data.position_cost, 0, 0), 2));
            return this;
        };
        F.prototype.onMonitorPosition_cost = function (t) {
            t.val(EFO.Checks.formatPriceNSD(U.FloatMoreEqOr(t.val(), 0, 0), 2));
            return this;
        };
        F.prototype.onMonitorNds_pc = function (t) {
            t.val(EFO.Checks.formatPriceNSD(U.FloatMoreEqOr(t.val(), 0, 0), 2));
            return this;
        };
        F.prototype.onMonitorNds_eur = function (t) {
            t.val(EFO.Checks.formatPriceNSD(U.FloatMoreEqOr(t.val(), 0, 0), 2));
            return this;
        };
        F.prototype.onMonitorRc = function () {
            var s = U.FloatMoreEqOr(this.getField('position_cost').val(),0,0);
            var n = U.FloatMoreEqOr(this.getField('nds_eur').val(),0,0);
            this.getRole('calc_result').html( EFO.Checks.formatPriceNSD( s+n,2 ) );
            return this;
        };
        ...
        F.prototype.save = function (keep_open) {
            ...
            this.showLoader();
            jQuery.post('/admin/Requests/API', post_data, null, 'json')

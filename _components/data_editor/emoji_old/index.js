(function () {
    ...
    function initPlugin() {
        ...
        F.prototype.onInit = function () {
            ...
            this.init_table();
            this.getRole('file').on('change', this.on_file_changed.bindToObject(this));
            return this;
        };

        F.prototype.on_file_changed = function () {
            if (!(window.File && window.FileReader)) {
                U.Error("browser not supported");
            }
            if (this.getRole('file').get(0).files.length) {
                var reader = new FileReader();
                reader.onload = this.on_new_file.bindToObject(this);
                reader.readAsText(this.getRole('file').get(0).files[0]);
            }
        };

        F.prototype.on_new_file = function (e) {
            var xml = U.NEString(e.target.result, '');
            var render_div = jQuery('<div></div>');
            render_div.html(xml);
            var svg = render_div.find('svg');
            if (svg && svg.length) {
                this.on_file_text_ready(svg.get(0).outerHTML);
                render_div.html('');
                this.getRole('file').val('');
            }
        };

        F.prototype.tmp_container = function () {
            if (!this._tmp_container) {
                this._tmp_container = jQuery('<div style="display:none"></div>');
                this._tmp_container.appendTo('body');
            }
            return this._tmp_container;
        };

        F.prototype.refresh_image = function () {
            this.getRole('preview').html(['<svg><use xlink:href="#a', MD, '_image_tmp_', '"></svg>'].join(''));
            return this;
        };

        F.prototype.on_file_text_ready = function (svg_text) {
            var t = jQuery(svg_text);
            t.attr('id', ["a", MD, "_image_tmp_"].join(''));
            this.tmp_container().html('');
            t.appendTo(this.tmp_container());
            this.getField('image').val(t.get(0).outerHTML);
            this.refresh_image();
            return this;
        };
        ...
        F.prototype.load = function (id) {
            ...
            if (U.IntMoreOr(id, 0, null)) {
                this.showLoader();
                jQuery.getJSON('/admin/EmojiList/API', {action: "get", id: id})
                ...
                this.showLoader();
                jQuery.getJSON('/admin/EmojiList/API', {action: "langlist"})
        ...
        F.prototype._set_field_image = function (d, fi) {            
            this.tmp_container().html('');
            this.getField('image').val('');
            var dd = U.NEString(d.image,null);
            if (dd) {
                this.getField('image').val(dd);
                var ddt = jQuery(dd);
                ddt.attr('id', ['a', MD, '_image_tmp_'].join(''));
                ddt.appendTo(this.tmp_container());
            }
            this.refresh_image();
        };

        F.prototype._get_field_image = function () {
            var tag = U.NEString(this.getField('tag').val(), null);
            if (tag) {
                var t = U.NEString(this.getField('image').val(), null);
                if (t) {
                    var jt = jQuery(t);
                    jt.attr('id', ["emoji_icon_", tag].join(''));
                    return jt.get(0).outerHTML;
                }
            }
            return null;
        };
        ...
        F.prototype.save = function (keep_open) {
            ...
            jQuery.post('/admin/EmojiList/API', post_data, null, 'json')

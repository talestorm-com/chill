(function () {
    ...
    function initPlugin() {
        ...
        F.prototype.getDefaultTitle = function () {
            return "Редактирование системы размеров";
        };
        //</editor-fold>                          
        ...
        F.prototype.load = function (id) {
            ...
            if (U.IntMoreOr(id, 0, null)) {
                this.showLoader();
                jQuery.getJSON('/admin/SizeVoc/API', {action: "get_size_system", id: id})
        ...
        F.prototype.save = function (keep_open) {
            ...
            if (EFO.Filter.Values().BadValue.is(data.short_name) && !EFO.Filter.Values().BadValue.is(data.name)) {
                data.short_name = data.name;
            } else if (!EFO.Filter.Values().BadValue.is(data.short_name) && EFO.Filter.Values().BadValue.is(data.name)) {
                data.name = data.short_name;
            }
            EFO.Filter.Filter().throwValuesErrorFirst(data, true);
            var post_data = {
                action: "post_size_system",
                data: JSON.stringify(data)
            };
            this.showLoader();
            jQuery.post('/admin/SizeVoc/API', post_data, null, 'json')

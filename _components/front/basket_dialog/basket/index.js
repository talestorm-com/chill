(function () {
    ...
    function initPlugin() {
        ...
        F.prototype.onCommandDo_order = function () {
            this.go_to_basket_after = true;
            this.do_work();
        };

        F.prototype.onCommandDo_basket_add = function () {
            this.go_to_basket_after = false;
            this.do_work();
        };


        F.prototype.do_work = function () {
            try {
                ...
                if (selected_sizes.length) {
                    for (var i = 0; i < selected_sizes.length; i++) {
                        if (!PM.get_filter_qty_of(this.product_info.product_id, null, this.selected_color, this.selected_sizes[i])) {
                            U.Error(["Размер \"", this.product_info.sizes.get_value_by_id(selected_sizes[i]), "\": нет в наличии"].join(''));
                        }
                    }
                } else {
                    if (!PM.get_filter_qty_of(this.product_info.product_id, null, this.selected_color, null)) {
                        U.Error("Товара нет в наличии");
                    }
                }
        ...
        F.prototype.on_post_responce = function (d) {
            // не забыть -респонс от баскета 
            d = U.safeObject(d);
            if (d.status === 'ok') {
                if (this.go_to_basket_after) {
                    window.location.href = "/Basket";
                } else {
                    EFO.Alert().set_style("blue")
                            .set_text("Товар добавлен в корзину")
                            .set_title("Выполнено")
                            .set_close_btn(true)
                            .set_icon("success")
                            .set_image("success")

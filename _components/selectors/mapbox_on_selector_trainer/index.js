(function () {
    ...
    function initPlugin() {
        ...
        F.prototype.onInit = function () {
            ...
            this.has_default_image = (function(x){
                return U.NEString(x.default_image,null)?true:false;
        ...
        F.prototype.on_marker_click = function (_t, e) {
            ...
            if (alias && U.isObject(this.markers[alias]) && U.isObject(this.markers[alias].xob)) {
                ...
                var old_selected_alias = ["P", this.selected].join('');
                this.selected = U.IntMoreOr(this.markers[alias].xob.id, 0, null);
                var om = this.markers[alias];
                var delegate = this.deledate;
                var new_marker = new mapboxgl.Marker({
                    draggable: false,
                    color: delegate.mbsd_get_marker_color(om.xob),
                }).setLngLat([om.getLngLat().lng, om.getLngLat().lat])
                        .addTo(this.map);
                new_marker.xob = om.xob;
                this.markers[alias] = new_marker;
                jQuery(new_marker.getElement()).data('alias', alias);
                jQuery(new_marker.getElement()).on('click', this.on_marker_click.bindToObjectWParam(this));
                om.remove();
                if (this.markers[old_selected_alias]) {
                    var om = this.markers[old_selected_alias];
                    var delegate = this.deledate;
                    var new_marker = new mapboxgl.Marker({
                        draggable: false,
                        color: delegate.mbsd_get_marker_color(om.xob),
                            ...
                            .addTo(this.map);
                    new_marker.xob = om.xob;
                    this.markers[old_selected_alias] = new_marker;
                    jQuery(new_marker.getElement()).data('alias', old_selected_alias);
                    jQuery(new_marker.getElement()).on('click', this.on_marker_click.bindToObjectWParam(this));
                    om.remove();
                }
            }
        ...
        F.prototype.mbsd_load_points = function (cb) {
        ...
        F.prototype.mbsd_get_marker_color = function (mi) {
            var id = U.IntMoreOr(mi.id, 0, null);
            if (this.selected === id) {
                return '#ff0000';
            }
        ...
        F.prototype.mbsd_ok = function () {
            var items_to_add = [];
            if (U.IntMoreOr(this.selected, 0, null)) {
                items_to_add.push(this.selected);
            }
            if (!items_to_add.length) {
                U.Error("Ничего не выбрано!");
            }
            this.showLoader()
            jQuery.post('/Cabinet/API', {action: "trainer_points_selected", points: items_to_add})
                    .done(this.vocload_response.bindToObject(this))
                    .fail(this.vocon_load_error.bindToObject(this))
                    .always(this.hideLoader.bindToObject(this));

        };
        F.prototype.mbsd_fill_popup = function (mi) {
            this.crip = mi;
            return Mustache.render(EFO.TemplateManager().get('popup', MC), this);
        };
        F.prototype.mbsd_on_command = function (cmd, src, mi) {

        };
        F.prototype.vocload_response = function (d) {
            if (U.isObject(d)) {
                if (d.status === "ok") {
                    if (U.isArray(d.items) && d.items.length) {
                        return this.onvocsuccess(d);
                    }
                    return this.nwf("Не найдено");
                }
                if (d.status === 'error') {
                    return this.nwf(d.error_info.message);
                }

            }
            return this.nwf("Некорректный ответ сервера");
        };
        F.prototype.nwf = function (x) {
            U.TError(U.NEString(x, "Ошибка сети"));
            return this;
        };
        F.prototype.vocon_load_error = function () {
            return this.nwf(null);
        };
        
        F.prototype.onvocsuccess = function(d){
            this.runCallback(d.items[0]);
            return this.hide().clear();
        };
        //</editor-fold>

(function () {
    ...
    function initPlugin() {
        ...
        F.prototype.getDefaultTitle = function () {
            return "Выбор Тренера";
        };
        //</editor-fold>                          
        //<editor-fold defaultstate="collapsed" desc="Лоадер">        
        F.prototype.load = function (id) {
            this.showLoader();
            jQuery.getJSON('/Info/API', {id: id, action: "trainers_for_hall"})
        ...
        F.prototype.on_response = function (d) {
            if (U.isObject(d)) {
                if (d.status === 'ok') {
                    return this.on_load_success(d.trainers);
                }
                if (d.status === 'error') {
        ...
        F.prototype.save = function (keep_open) {
            ...
            var id = U.IntMoreOr(jQuery(f.get(0)).data('id'), 0, null);
            if (id) {
                var t = null;
                var tt = U.safeArray(this.trainers);
                for(var i = 0;i<tt.length;i++){
                    if(U.IntMoreOr(tt[i].id,0,null)===id){
                        t = tt[i];
                        break;
                    }
                }                

    ...
    function initPlugin() {
        ...
        F.prototype.sync_selection = function (s) {
            ...
            for (var i = 0; i < all.length; i++) {
                ...
                var id = U.IntMoreOr(cb.data('id'), 0, null);
                if (id && s.exists(id)) {
        ...
        F.prototype.getFooterButtons = function () {
            return [
                {'command': "add", 'text': "Добавить", buttonClass: MC + "AddButton"},
        ...
        F.prototype.table_def = function () {
            return {
                ...
                columns: [
                    ...
                    {id: "glyph", key: "glyph", property: "glyph", "text": "Глиф", filter: false, sort: false},
                    {id: "tag", key: "tag", property: "tag", "text": "Tag", filter: "String", sort: true},
        ...
        F.prototype.init_table = function () {
            ...
            var DSParams = ADVT.DataSource.SimplePostParams({'url': '/admin/EmojiList/API?action=list', method: 'post'}, ADVT.DataSource.Extractor.MixExtractor({}));
        ...
        F.prototype.on_data_ready_pre = function (data) {            
            if (U.isObject(data)) {
                if (data.status === 'ok') {
                    if (U.isObject(data.voc)) {
                        if (!this.svg_handle) {
                            this.svg_handle = jQuery("<div style=\"display:none\"></div>");
                            this.svg_handle.appendTo('body');
                        }
                        this.svg_handle.html('');
                        var tmp_handle = jQuery('<div></div>');
                        for (var k in data.voc) {
                            if (data.voc.hasOwnProperty(k) && U.isObject(data.voc[k]) && data.voc[k].image && data.voc[k].tag === k) {
                                tmp_handle.html(data.voc[k].image);
                                var svg = tmp_handle.find('svg');
                                if (svg && svg.length) {
                                    svg.attr("id", ['a', MD, "_emo_", k].join(''));
                                    svg.appendTo(this.svg_handle);
                                }
                            }
                        }
                    }
        ...
        F.prototype.on_request_params = function (a, b, c) {
            //var rc_id = U.IntMoreOr(this.getRole('catalog_filter_id').val(), 0, null);
            //a.filters.catalog = rc_id;
            return this;
        };

        F.prototype.get_item_cloned = function (x) {
            var data = U.safeArray(this.table.body.Renderer.rows);
            var search_id = U.IntMoreOr(x, 0, null);
            if (search_id) {
                for (var i = 0; i < data.length; i++) {
                    if (U.IntMoreOr(data[i].id, 0, null) === search_id) {
                ...
                if (id) {
                    if (this._shift) {
                        ...
                        this.handle.find('input[type=checkbox]').each(function () {
                            ...
                            var id = U.IntMoreOr(c.data('id'), 0, null);
                            if (i && id) {
...
//            this.getRole('catalog_filter').val(U.NEString(o.n, null));
        ...
        F.prototype.on_editor_ready = function (x) {
            x.show().load(null).setCallback(this, this.load);
            return this;
        };

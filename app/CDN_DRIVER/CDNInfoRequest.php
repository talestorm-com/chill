...
class CDNInfoRequest {
    ...
    protected function mk_request(array $data, string $method = 'GET') {
        ...
        $query_hash = "{$method}+api.platformcraft.ru/1/objects/{$this->id}?" . implode("&", $encoded_data);
    ...
    public function run(string $cdn_id) {
        ...
        $this->id = $cdn_id;
        $url = "https://api.platformcraft.ru/1/objects/{$cdn_id}?" . $this->mk_request($data, 'GET');

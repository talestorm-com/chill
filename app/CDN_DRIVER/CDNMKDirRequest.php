...
class CDNMKDirRequest {
    ...
    public function run($dir_name) {
        ...
        $dname = CDNPathRecoverer::recover_path($this->path . "/{$dir_name}/mkdir.null");
        ...
        $fields = [
            'file' => new \CurlFile(__FILE__, 'application/octet-stream', 'file.bin'),
            'name' => $dname,
        ];
            ...
            if (is_array($this->result)) {
                ...
                if ($result_map->get_filtered("status", ['Strip', 'Trim', 'NEString', 'DefaultNull']) === "success") {
                    if ($result_map->get_filtered("code", ['IntMore0', 'DefaultNull']) === 200) {
                        ...
                        if ($object_id) {
                            CDNRemoveRequest::F()->run($object_id);
                        }
                    }
                }
            }

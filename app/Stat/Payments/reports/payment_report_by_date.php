...
class payment_report_by_date {
    ...
    protected function init_xlsx() {
        ...
        $this->xlsx = \PhpOffice\PhpSpreadsheet\IOFactory::load(__DIR__ . DIRECTORY_SEPARATOR . "payment_report_2.xlsx");
    ...
    protected function parse_cord($cord, &$cord_letter, &$cord_number) {
        $m = [];
        if (preg_match("/^(?P<letter>[A-Z]{1,})(?P<number>\d{1,})$/i", $cord, $m)) {
            $cord_letter = trim($m['letter']);
            $cord_number = intval($m['number']);
        }
    }

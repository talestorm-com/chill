...
class Writer {
    ...
    public function run() {
        ...
        if ($data['id']) {
            ...
            $b->push("UPDATE media__content__tag SET sort=:P{$b->c}sort WHERE id={$t};");
            $b->push_param(":P{$b->c}id", $data['id']);
        } else {
            $b->push("INSERT INTO media__content__tag (sort) VALUES(:P{$b->c}sort);");
        ...
        $b->inc_counter();
        $b->push("DELETE FROM media__content__tag__strings WHERE id={$t}; ");
        $b->inc_counter();
        $params = [];
        $c = 0;
        $inserts = [];
        ...
        if (count($inserts)) {
            $b->push(sprintf("INSERT INTO media__content__tag__strings(id,language_id,name) VALUES %s ON DUPLICATE KEY UPDATE name=VALUES(name);", implode(",", $inserts)))

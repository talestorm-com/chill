...
class Lister extends \Content\Lister\Lister {
    ...
    protected function build_query() {
        return sprintf("SELECT SQL_CALC_FOUND_ROWS A.id,A.sort,COALESCE(B.name,C.name) name,A.tag FROM
            media__emoji A
            LEFT JOIN media__emoji__strings B ON(A.id=B.id AND B.language_id='%s')
            LEFT JOIN media__emoji__strings C ON(A.id=C.id AND C.language_id='%s')
            %s %s %s %s
            ",

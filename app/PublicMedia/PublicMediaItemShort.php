 ...
 */
class PublicMediaItemShort implements \common_accessors\IMarshall {
    ...
    public static function F(): PublicMediaItemShort {
        return new static();
    }
    ...
    protected static function query_proto(): string {
        return "SELECT A.id,B.owner_id,A.title,A.active,A.type,A.extension,A.aspect,A.preview_aspect,A.sort,A.created,
            A.gallery_id,B.visible gallery_visible
     ...
     */
    public function load(int $id): PublicMediaItemShort {
     ...
     * @return \PublicMedia\PublicMediaItemShort
     */
    public function load_array(array $data): PublicMediaItemShort {
     ...
     */
    public static function load_by_gallery(int $gallery_id): array {
        $qs = sprintf(static::query_proto(), "A.gallery_id=:PP ORDER BY sort,created DESC,A.id");
        $rows = \DB\DB::F()->queryAll($qs, [":PP" => $gallery_id]);
     ...
     * 
     * @param array $data
     * @return PublicMediaItemShort|null
     */

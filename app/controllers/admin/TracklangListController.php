 ...
 * @author eve
 */
class TracklangListController extends AbstractAdminController {

    public function get_desktop_component_id() {
        return "desktop.tracklang_list";
    }
    ...
    public function API_list() {
        \Language\TracklangLister::F(\DataMap\PostDataMap::F())->run($this->out);
    ...
    public function API_get(string $rid = null) {
        ...
        $item = \Language\TrackLang::F()->load($id);
        $item && $item->valid ? 0 : \Errors\common_error::R("not found");
        $this->out->add('data', $item);
        $this->API_langlist();
    }
    ...
    public function API_put() {
        ...
        $ret_id = \Language\TrackLangWriter::F(\DataMap\CommonDataMap::F()->rebind($data))->run()->id;
        $this->API_get($ret_id);
    }
    ...
    public function API_remove() {
        ...
        if ($id) {
            \DB\SQLTools\SQLBuilder::F()->push("DELETE FROM media__content__tracklang WHERE id=:P")->push_param(":P", $id)->execute_transact();
        }
        $this->API_list();
    }

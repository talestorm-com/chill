 ...
 * @author eve
 */
class ProfilesController extends AbstractAdminController {

    public function get_desktop_component_id() {
        return "desktop.requests.profile";
    }
    ...
    protected function API_list() {
        \Content\RequestProfile\Lister::F(\DataMap\PostDataMap::F())->run($this->out);
    }
    ...
    protected function API_get(int $rid = null) {
        ...
        $row = \DB\DB::F()->queryRow("SELECT * FROM request__profile WHERE id=:P", [":P" => $id]);
    ...
    protected function API_post() {
        ...
        $writer = \Content\RequestProfile\Writer\Writer::F($datamap, $datamap);
        $writer->run();
    ...
    protected function API_remove() {
        ...
        if ($id_to_remove) {
            \DB\DB::F()->exec("DELETE FROM request__profile WHERE id=:P", [":P" => $id_to_remove]);
        }

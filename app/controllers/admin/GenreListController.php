 ...
 * @author eve
 */
class GenreListController extends AbstractAdminController {

    public function get_desktop_component_id() {
        return "desktop.genre_list";
    }
    ...
    public function API_list() {
        \Genre\Lister::F(\DataMap\PostDataMap::F())->run($this->out);
    }
    ...
    public function API_put() {
        ...
        $ret_id = \Genre\GenreWriter::F(\DataMap\CommonDataMap::F()->rebind($data))->run()->id;
        $this->API_get($ret_id);
    }
    ...
    public function API_remove() {
        ...
        if ($id) {
            \DB\SQLTools\SQLBuilder::F()->push("DELETE FROM media__content__genre WHERE id=:P")->push_param(":P", $id)->execute_transact();
        }
        $this->API_list();
    }

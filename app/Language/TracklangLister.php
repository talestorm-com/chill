 ...
 * @author eve
 */
class TracklangLister extends \Content\Lister\Lister {
    ...
    protected function build_query() {
        return sprintf("SELECT SQL_CALC_FOUND_ROWS A.id,COALESCE(B.name,C.name) name
            ...
            LEFT JOIN media__content__tracklang__strings B ON(A.id=B.id AND B.language_id='%s')
            LEFT JOIN media__content__tracklang__strings C ON(A.id=C.id AND C.language_id='%s')
            %s %s %s %s
            ",

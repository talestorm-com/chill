 ...
 * @author studio2
 */
class TaskRestorePasswordPhaseTwo extends \AsyncTask\AsyncTaskAbstract {
    ...
    protected function exec() {
        try {
            ...
            $new_password = $this->params->get_filtered("new_password", ["Strip", "Trim", "NEString", "DefaultNull"]);
            if ($user_login) {
                $user_info = \Auth\UserInfo::S($user_login);
                if ($user_info && $user_info->valid) {
                    \SWIFT\SWIFTMAILER::F()->send_email_with_template("restore/phase2", $user_info->login, "Восстановление пароля", [
                        'user_info' => $user_info,

...
/**
 * Description of FilterPresetItem
 *
 * @author eve
 ...
 * @property string $preset  preset text
 ...
 */
class FilterPresetItem implements \common_accessors\IMarshall {

    use \common_accessors\TCommonAccess,
        \common_accessors\TDefaultMarshaller,
        \common_accessors\TCommonImport;

    const MEDIA_CONTEXT = "preset_item";
    ...
    /** @var int */
    protected $marshall_mode = FilterPreset::MARSHALL_MODE_FULL;
    ...
    protected function __get__valid() {
        return !!($this->id && $this->uid && $this->name && $this->preset);
    }
    ...
    public function __wakeup() {
        $this->marshall_mode = FilterPreset::MARSHALL_MODE_FULL;
    }

    public function __construct(array $data = null) {
        $this->class_version = static::$cv;
        if ($data) {
            $this->import($data);
        }
    }
     ...
     * 
     * @param array $data
     * @return \Content\FilterPreset\FilterPresetItem
     */
    public function import(array $data): FilterPresetItem {
     ...
     * @return \Content\FilterPreset\FilterPresetItem
     */
    public static function F(array $data = null): FilterPresetItem {
    ...
    protected function t_common_import_get_filters(): array {
        return [
            ...
            'preset' => ['Trim', 'NEString', 'DefaultNull'],
    ...
    protected function t_default_marshaller_on_props_to_marshall(array &$props) {
        if ($this->marshall_mode === FilterPreset::MARSHALL_MODE_TRIM) {
            if (array_key_exists("preset", $props)) {
                unset($props["preset"]);
            }
    ...
    public static function C(int $id, string $uid): FilterPresetItem {
        ...
        $item = static::F();
        $item->load_id($id, $uid);
        \Cache\FileCache::F()->put($cache_key, $item, 0, \Cache\FileBeaconDependency::F(FilterPreset::CACHE_BEAKON));
        return $item;
    }

    public static function C2(string $uid): FilterPresetItem {
     ...
     * @return \Content\FilterPreset\FilterPresetItem
     */
    protected function load_id(int $id, string $uid): FilterPresetItem {
        $query = "SELECT A.*,B.name package_name,B.cost package_cost FROM filterpreset__item A JOIN filterpreset B ON(A.id=B.id) WHERE A.id=:P AND A.uid=:PP AND B.active=1";
     ...
     * @return \Content\FilterPreset\FilterPresetItem
     */
    protected function load_uid(string $uid): FilterPresetItem {
        $query = "SELECT A.*,B.name package_name,B.cost package_cost FROM filterpreset__item A JOIN filterpreset B ON(A.id=B.id) WHERE  A.uid=:PP AND B.active=1";

...
class RibbonLent extends \Content\Content {
    ...
    CONST PAGE_MULTER = 2;
    CONST SEASONS_PER_PAGE = 36 * 2;
    CONST COLLECTIONS_PER_PAGE = 11 * 2;
    ...
    protected function load(\Language\LanguageItem $language, int $page) {
        ...
        for ($i = 0; $i < static::PAGE_MULTER; $i++) {
            $result = array_merge($result, array_slice($seasons, $i * 36, 4));
            $result = array_merge($result, array_slice($seasons, $i * 36 + 4, 4));
            $result = array_merge($result, array_slice($collections, $i * 11, 2));
            $result = array_merge($result, array_slice($seasons, $i * 36 + 8, 4));
            $result = array_merge($result, array_slice($seasons, $i * 36 + 12, 4));
            $result = array_merge($result, array_slice($collections, $i * 11 + 2, 3));
            $result = array_merge($result, array_slice($seasons, $i * 36 + 16, 4));
            $result = array_merge($result, array_slice($seasons, $i * 36 + 20, 4));
            $result = array_merge($result, array_slice($collections, $i * 11 + 5, 2));
            $result = array_merge($result, array_slice($seasons, $i * 36 + 24, 4));
            $result = array_merge($result, array_slice($collections, $i * 11 + 7, 2));
            $result = array_merge($result, array_slice($seasons, $i * 36 + 28, 4));
            $result = array_merge($result, array_slice($collections, $i * 11 + 9, 1));
            $result = array_merge($result, array_slice($seasons, $i * 36 + 32, 4));
            $result = array_merge($result, array_slice($collections, $i * 11 + 10, 1));
        }
        $this->items = $result;
    }

...
class RibbonLent extends \Content\Content {
    ...
    CONST PAGE_MULTER = 3;
    CONST SEASONS_PER_PAGE = 12 * 3;
    CONST GIFTRAILS_PER_PAGE = 5 * 3;
    ...
    protected function load(\Language\LanguageItem $language, int $page) {
        ...
        for ($i = 0; $i < static::PAGE_MULTER; $i++) {
            $result = array_merge($result, array_slice($seasons, $i * 12, 6));
            $result = array_merge($result, array_slice($giftrailer, $i * 5, 2));
            $result = array_merge($result, array_slice($banners, $i, 1));
            $result = array_merge($result, array_slice($collections, $i, 1));
            $result = array_merge($result, array_slice($seasons, $i * 12 + 6, 6));
            $result = array_merge($result, array_slice($giftrailer, $i * 5 + 2, 3));
        }
        $this->items = $result;
    }

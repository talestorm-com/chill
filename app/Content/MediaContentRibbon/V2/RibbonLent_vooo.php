...
class RibbonLent extends \Content\Content {
    ...
    CONST PAGE_MULTER = 2;
    CONST SEASONS_PER_PAGE = 16 * 2;
    // CONST GIFTRAILS_PER_PAGE = 2 * 3;
    CONST BANNERS_PER_PAGE = 1 * 2;
    CONST COLLECTIONS_PER_PAGE = 8 * 2;
    ...
    protected function load(\Language\LanguageItem $language, int $page) {
        ...
        for ($i = 0; $i < static::PAGE_MULTER; $i++) {
            $result = array_merge($result, array_slice($seasons, $i * 16, 4));
            $result = array_merge($result, array_slice($collections, $i*8, 3));
            $result = array_merge($result, array_slice($seasons, $i * 16+4, 4));
            $result = array_merge($result, array_slice($banners, $i*1, 1));
            $result = array_merge($result, array_slice($collections, $i*8+3, 1));
            $result = array_merge($result, array_slice($seasons, $i * 16+8, 4));
            $result = array_merge($result, array_slice($collections, $i*8+4, 2));
            $result = array_merge($result, array_slice($seasons, $i * 16+12, 4));
            $result = array_merge($result, array_slice($collections, $i*8+6, 1));
            $result = array_merge($result, array_slice($collections, $i*8+7, 1));
        }
        $this->items = $result;
    }

...
/**
 * Description of MediaContentRibbonTagGIF
 *
 * @author eve
 * @property string $file_version
 * @property MediaContentRibbonItem[] $items
 */
class MediaContentRibbonTagGIF implements \common_accessors\IMarshall, \Countable, \Iterator {
    ...
    protected function __construct(int $tag_id, int $offset, int $perpage, \Language\LanguageItem $language, \Language\LanguageItem $default_language) {
        $this->file_version = static::get_file_version();
        $this->items = [];
        $this->load($tag_id, $offset, $perpage, $language, $default_language);
        $this->cache($tag_id, $offset, $perpage, $language);
    }

    protected function cache(int $tag_id, int $offset, int $perpage, \Language\LanguageItem $language) {
        $cache = \Cache\FileCache::F();
        $cache_key = static::mk_cache_key($tag_id, $offset, $perpage, $language);
        //
    }

    protected static function mk_cache_key(int $tag_id, int $offset, int $perpage, \Language\LanguageItem $language) {
        return implode("---", [__CLASS__, $tag_id, $offset, $perpage, $language->id]);
    }

    protected function load(int $tag_id, int $offset, int $perpage, \Language\LanguageItem $language, \Language\LanguageItem $default_language) {
        $query = "
            ...
            JOIN media__content__tag__list ATAG ON(ATAG.media_id=A.id AND ATAG.tag_id=:Ptag)
        ...
        $rows = \DB\DB::F()->queryAll($aquery, [":Ptag" => $tag_id]);
        $this->items = [];
        foreach ($rows as $row) {
            try {

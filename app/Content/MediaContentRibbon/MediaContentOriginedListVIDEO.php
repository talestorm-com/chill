 ...
 */
class MediaContentOriginedListVIDEO implements \common_accessors\IMarshall, \Countable, \Iterator {
    ...
    protected function __construct(int $origin_id, int $offset, int $perpage, \Language\LanguageItem $language, \Language\LanguageItem $default_language) {
        $this->file_version = static::get_file_version();
        $this->items = [];
        $this->load($origin_id, $offset, $perpage, $language, $default_language);
        $this->cache($origin_id, $offset, $perpage, $language);
    }

    protected function cache(int $origin_id, int $offset, int $perpage, \Language\LanguageItem $language) {
        $cache = \Cache\FileCache::F();
        $cache_key = static::mk_cache_key($origin_id, $offset, $perpage, $language);
        //
    }

    protected static function mk_cache_key(int $origin_id, int $offset, int $perpage, \Language\LanguageItem $language) {
        return implode("---", [__CLASS__, $origin_id, $offset, $perpage, $language->id]);
    }

    protected function load(int $origin_id, int $offset, int $perpage, \Language\LanguageItem $language, \Language\LanguageItem $default_language) {
        $query = "
            ...
            JOIN media__content__origin OL ON(OL.id=A.id AND OL.country_id=:Porigin)
        ...
        $rows = \DB\DB::F()->queryAll($aquery, [":Porigin" => $origin_id]);
        $this->items = [];
        foreach ($rows as $row) {
            try {

 ...
 */
class MediaContentLangSplitListSOAP implements \common_accessors\IMarshall, \Countable, \Iterator {
    ...
    protected function __construct(int $language_id, int $offset, int $perpage, \Language\LanguageItem $language, \Language\LanguageItem $default_language) {
        $this->file_version = static::get_file_version();
        $this->items = [];
        $this->load($language_id, $offset, $perpage, $language, $default_language);
        $this->cache($language_id, $offset, $perpage, $language);
    }

    protected function cache(int $language_id, int $offset, int $perpage, \Language\LanguageItem $language) {
        $cache = \Cache\FileCache::F();
        $cache_key = static::mk_cache_key($language_id, $offset, $perpage, $language);
        //
    }

    protected static function mk_cache_key(int $language_id, int $offset, int $perpage, \Language\LanguageItem $language) {
        return implode("---", [__CLASS__, $language_id, $offset, $perpage, $language->id]);
    }

    protected function load(int $language_id, int $offset, int $perpage, \Language\LanguageItem $language, \Language\LanguageItem $default_language) {
        $query = "
            ...
            WHERE A.enabled=1 AND A.track_language=:P
            ORDER BY A.id DESC
        ...
        $rows = \DB\DB::F()->queryAll($aquery, [":P" => $language_id]);
        $this->items = [];
        foreach ($rows as $row) {
            try {
     ...
     * @return \static
     */
    public static function F(int $language_id, int $offset, int $perpage, \Language\LanguageItem $language, \Language\LanguageItem $default_language) {
        $cs = static::class;
        $cache = \Cache\FileCache::F();
        $cache_key = static::mk_cache_key($language_id, $offset, $perpage, $language);
        ...
        return new static($language_id, $offset, $perpage, $language, $default_language);
    }

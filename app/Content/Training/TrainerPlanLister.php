 ...
 * @author eve
 */
class TrainerPlanLister extends \Content\Lister\Lister {
    ...
    protected function build_query() {
        return sprintf("SELECT  SQL_CALC_FOUND_ROWS  *, DATE_FORMAT(DATE_ADD(datum, INTERVAL MOD(start,86400) SECOND),'%%H:%%i') `time`,
            ...
            %s %s %s %s            
            ", $this->filter->whereWord, $this->where, " ORDER BY datum , start  ", $this->limit->MySqlLimit);
    }
    ...
    protected function create_direct_conditions() {
        ...
        $this->filter->addDirectCondition("(trainer_id=:PXuser)");
        $this->filter->addDirectCondition("(state!=2 AND datum>=:PXdate AND DATE_ADD(datum, INTERVAL MOD(start,86400) SECOND)>=:PXdate)");        
    }

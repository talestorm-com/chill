...
namespace Content\RequestProfile;
...
class Lister extends \Content\Lister\Lister {
    ...
    protected function get_sorts(): array {
        return [
            ...
            'enabled' => "A.enabled|A.sort|A.name|A.id",
            'sort' => "A.sort|A.id",
        ];
    }

    protected function build_query() {
        return sprintf("SELECT SQL_CALC_FOUND_ROWS A.id,A.name,A.color,A.enabled,A.sort FROM request__profile A %s %s %s %s",

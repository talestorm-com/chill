...
namespace Content\RequestProfile\Writer;
...
class CommonWriter {
    ...
    protected function create_sql(array $data, Writer $w) {        
        $w->builder->push_params([
            ...
            ":P{$w->builder->c}enabled" => $data["enabled"] ? 1 : 0,            
        ]);
        if ($data['id']) {
            $w->builder->push("SET {$w->temp_var} = :P{$w->builder->c}id;");
            $w->builder->push_param(":P{$w->builder->c}id", $data["id"]);
            $w->builder->push("UPDATE request__profile SET name=:P{$w->builder->c}name,color=:P{$w->builder->c}color,
                enabled=:P{$w->builder->c}enabled,sort=:P{$w->builder->c}sort
                WHERE id={$w->temp_var};    
            ");
        } else {
            $w->builder->push("INSERT INTO request__profile(name,color,enabled,sort )
                VALUES(:P{$w->builder->c}name,:P{$w->builder->c}color,:P{$w->builder->c}enabled,:P{$w->builder->c}sort);
                ");

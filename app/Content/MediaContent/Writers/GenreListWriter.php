...
class GenreListWriter {
    ...
    public function run(AWriter $writer) {
        $genre_list_raw = $writer->input->get_filtered("genres", ["NEArray", "DefaultEmptyArray"]);
        $genre_lis = [];
        ...
        $writer->builder->push("DELETE FROM media__content__genre_list WHERE media_id={$writer->temp_var};");
        $writer->builder->inc_counter();
        if (count($genre_lis)) {
            $i = [];
            $c = 0;
            ...
            if (count($i)) {
                $writer->builder->push(sprintf("INSERT INTO media__content__genre_list(media_id,genre_id,sort) VALUES %s ON DUPLICATE KEY UPDATE sort=VALUES(sort);", implode(",", $i)))

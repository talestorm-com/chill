        ...
        if (count($itw)) {
            ...
            foreach ($itw as $item) {
                ...
                $i[] = "({$writer->temp_var},
                    ...
                    :P{$writer->builder->c}_i{$c}_selector,
                    :P{$writer->builder->c}_i{$c}_content_type,
                    :P{$writer->builder->c}_i{$c}_size,
                    :P{$writer->builder->c}_i{$c}_sort,
                    :P{$writer->builder->c}_i{$c}_info)";
                $p = array_merge($p, [
                    ":P{$writer->builder->c}_i{$c}_cdn_id" => $item["cdn_id"],
                    ":P{$writer->builder->c}_i{$c}_enabled" => $item['enabled'],
                    ":P{$writer->builder->c}_i{$c}_selector" => $item['selector'],
                    ":P{$writer->builder->c}_i{$c}_content_type" => $item["content_type"],
                    ":P{$writer->builder->c}_i{$c}_size" => $item["size"],
                            ":P{$writer->builder->c}_i{$c}_sort" => $item["sort"],
                    ":P{$writer->builder->c}_i{$c}_info" => $item["info"],
                ]);
                $c++;
            }
            if (count($i)) {
                $writer->builder->push(sprintf("INSERT INTO media__content__cdn__file(id,cdn_id,`enabled`,`selector`,content_type,size,sort,info) VALUES %s 
                   ON DUPLICATE KEY UPDATE `enabled`=VALUES(`enabled`),content_type=VALUES(content_type),size=VALUES(size),info=VALUES(info),`selector`=VALUES(`selector`),sort=VALUES(sort);

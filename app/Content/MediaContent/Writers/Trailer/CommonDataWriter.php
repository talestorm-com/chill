    ...
    public function run(Writer $writer) {
        ...
        if ($raw_data['id']) {
            ...
            $writer->builder->push("UPDATE media__content SET enabled=:P{$writer->builder->c}enabled,age_restriction=NULL,emoji=NULL,
                track_language=:P{$writer->builder->c}track_language,series_count=:P{$writer->builder->c}series_count,
                seasons_count=:P{$writer->builder->c}seasons_count, free=:P{$writer->builder->c}free
            ...
            $writer->builder->push("INSERT INTO media__content(alias,ctype,enabled,age_restriction,emoji,track_language,series_count,seasons_count,free) VALUES(
                UUID(),:P{$writer->builder->c}ctype,:P{$writer->builder->c}enabled,NULL,NULL,:P{$writer->builder->c}track_language,
                :P{$writer->builder->c}series_count,:P{$writer->builder->c}seasons_count,:P{$writer->builder->c}free

...
class CommonWriter {
    ...
    public function run(Writer $writer) {
        ...
        if ($raw_data['id']) {
            ...
            $writer->builder->push("INSERT INTO media__content(alias,ctype,enabled,age_restriction,emoji,mcsort) VALUES(
                UUID(),:P{$writer->builder->c}ctype,:P{$writer->builder->c}enabled,:P{$writer->builder->c}age_restriction,:P{$writer->builder->c}emoji,
                :P{$writer->builder->c}mcsort

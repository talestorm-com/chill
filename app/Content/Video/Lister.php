...
class Lister {
    ...
    protected function get_query_proto() {
        return "SELECT SQL_CALC_FOUND_ROWS A.id,A.alias,A.name,DATE_FORMAT(created,'%%d.%%m.%%Y') created,DATE_FORMAT(updated,'%%d.%%m.%%Y')updated
            ,DATE_FORMAT(published,'%%d.%%m.%%Y')published,A.active,A.cost,COALESCE(B.qty,0) qty,A.default_image FROM video__group A
            LEFT JOIN video__group__qty B ON(A.id=B.id)
            %s %s %s %s;";
    }

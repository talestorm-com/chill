...
class CommonWriter {
    ...
    /**
     * 
     * @return \Content\Video\Writer\CommonWriter
     */
    ...
    protected function create_sql(array $data, VideoGroupWriter $w) {
        $data['alias'] = \Helpers\Helpers::uniqueAlias('video__group', \Helpers\Helpers::NEString($data['alias'], \Helpers\Helpers::translit($data['name'])), $data['id']);
        ...
        if ($data['id']) {
            ...
            $w->builder->push("UPDATE video__group SET name=:P{$w->builder->c}name,alias=:P{$w->builder->c}alias,
            ...
            $w->builder->push("INSERT INTO video__group( name,alias,active,html_mode,cost,default_image,info )

...
class ItemsWriter {
    ...
    public function run(VideoGroupWriter $w) {
        ...
        $ic = 0;
        $bc = $w->builder->c;
        $query_prototype = "INSERT INTO video__group__item (id,uid,name,image,sort,html_mode,info) VALUES %s ON DUPLICATE KEY UPDATE
            name=VALUES(name),image=VALUES(image),sort=VALUES(sort),html_mode=VALUES(html_mode),info=VALUES(info);
            ";
        foreach ($items as $item) {
            ...
            $inserts[] = "({$w->temp_var},:P{$bc}_{$ic}_uid,:P{$bc}_{$ic}_name,:P{$bc}_{$ic}_image,:P{$bc}_{$ic}_sort,:P{$bc}_{$ic}_html_mode,:P{$bc}_{$ic}_info)";

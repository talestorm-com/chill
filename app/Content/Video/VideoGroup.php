 ...
 * @property \Content\PropertyCollection\AbstractPropertyCollection $properties
 * @property VideoItem[] $items
 * @property \Content\IImageCollection $images
 * @property string $class_version
 * @property int $marshall_mode
 */
class VideoGroup extends \Content\Content implements \Content\IImageSupport {

    use \common_accessors\TCommonImport;

    CONST MEDIA_CONTEXT = 'videogroup';
    CONST CACHE_BEAKON = 'videogroup';
    CONST MARSHALL_MODE_FULL = 0;
    CONST MARSHALL_MODE_TRIM = 1;
    CONST ACCESS_KEY = "VTR";
    ...
    /** @var \Content\PropertyCollection\AbstractPropertyCollection */
    ...
    public static function get_class_ver(): string {
        return md5(implode(",", [__FILE__, filemtime(__FILE__), VideoItem::get_class_ver()]));
    }

    /**
     * 
     * @return \Content\Video\VideoGroup
     */
    public static function F(): VideoGroup {
        return new static();
    }

    public function __construct() {
        $this->properties = VideoGroupPropertyCollection::F();
        $this->images = \Content\DefaultImageCollection::F();
        $this->items = [];
        $this->class_version = static::get_class_ver();
    }

    public function load(int $id): VideoGroup {
        $query = "SELECT * FROM video__group WHERE id=:P";
    ...
    protected function t_common_import_after_import() {
        if ($this->id && $this->name && $this->created && $this->guid) {
            ...
            $this->properties = VideoGroupPropertyCollection::F()->load_from_database($this->id);
            $this->load_items();
        }
    }
    ...
    protected function load_items() {
        $this->items = [];
        if ($this->id) {
            $query = "SELECT A.*,B.name package_name,B.cost package_cost FROM video__group__item A JOIN video__group B ON(A.id=B.id) WHERE A.id=:P ORDER BY A.sort,A.uid";
            $rows = \DB\DB::F()->queryAll($query, [":P" => $this->id]);
            foreach ($rows as $row) {
                $item = VideoItem::F($row);
     ...
     * @return \Content\Video\VideoGroup
     */
    public static function C(int $id): VideoGroup {
        $cache_key = implode("_", [static::class, $id]);
        $cached_value = \Cache\FileCache::F()->get($cache_key);
        if ($cached_value && ($cached_value instanceof VideoGroup) && $cached_value->class_version === static::get_class_ver()) {
            return $cached_value;
        }
        $value = static::F();
        $value->load($id);
     ...
     * @return VideoItem
     */
    public function get_item_by_image_file(string $video_name){
        foreach ($this->items as $item){
            if(strcasecmp($item->video, $video_name)===0){
                return $item;
            }
        }
        return null;

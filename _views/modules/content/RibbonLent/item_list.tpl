...
<div class="row">
    ...
    {assign var='absolute_index' value=$this->get_index_remainder(50)}{*47 сюда переехало из условия индекса*}
    <div class="abindexmarker" data-id="{$absolute_index}"></div>
    {assign var='index' value=0}z
    {foreach from=$this->items item='item'}        
        {if $index>49}
            {assign var='index' value=0}
        {/if}
        {if $absolute_index>49}
            {assign var='absolute_index' value=0}
        {/if}
                        ...
                        {else if $item->lent_mode==='video'}
                            <div class="film_left">
                                {if $item->age_restriction_name ==='0+'}
                                    <div class="age_age">{$item->age_restriction_name}</div>
                                {/if}
                                <div class="run_trailer" data-id='{$item->id}' data-srca='/Soap/{$item->id}-{$item->translit_name}' data-video_url="{$item->video_cdn_url}" data-video-id="{$item->video_cdn_id}">

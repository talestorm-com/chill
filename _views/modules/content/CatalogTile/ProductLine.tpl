{if count($this->items)}
    ...
    <div class="TileBlock TileBlock-{$this->tile->template} TileBlock-custom-{$this->tile->css_class}" id="tileblock_{$tileblock_uuid}">
        <div class="TileBlockInner">
            {if $this->tile->show_header}
                <div class="TileBlockHeader">{$this->tile->title}
            ...
            <div class="TileBlockContentFrame">
                <div class="TileBlockContent" style="width:{(18.64*count($this->items)+1.7*(count($this->items)-1))|format_percent:'vw'}">
                    {assign var="image_specification" value="SW_415H_622{if !$this->tile->crop}C_0{/if}{if $this->tile->crop_fill}CF_1{/if}{if $this->tile->background}B_{$this->tile->background_url}{/if}"}
                    {foreach $this->items as $product}
                        {if ($product->enabled || $this->tile->ignore_product_visibility)}

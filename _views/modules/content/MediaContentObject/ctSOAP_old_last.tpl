        ...
        <div class="container">
            <div class="row">
                <div class="col s12 m10 offset-m1">
                    <div id="cluii">
                        <div id="a_film_head_block">
                            ...
                            <div class="a-film-cover">
                                <img src="{$this->images->get_image_by_index(0)->image}.SW_1000H_600CF_1.jpg">
                    ...
                    <div id="film_btns">
                        <div class="row">
                            <div class="col s10 l8 offset-l1 offset-s1">
                                    ...
                                    <div class="season-tab-item" data-id="a{$season->id}">
                                        ...
                                        <div class="chill-season-series-list owl-carousel" id="owl-series">
                                            {foreach from=$season->series item='serie'}                                        
                                                <div class="chill-season-series-list-item" data-serie-id="{$serie->id}">
                    ...
                    <div class="row">
                        ...
                        <div class="col s11 l3 offset-s1">
                            <div id="a_film_features">
                                ...
                                <div class="one_features">Жанр: <b>{foreach from=$this->genres item='genre'}<a href="/search/by_genre/{$genre->id}">{$genre->name}</a> {/foreach}</b></div>
                                {if (count($this->persons->filter('ROLE_DIRECTOR')))}
                                    <div class="one_features">Режиссер: {foreach from=$this->persons->filter('ROLE_DIRECTOR') item='person' name='per'}<b>{$person->name}{if $smarty.foreach.per.last}{else},{/if}</b> {/foreach}</div>
                                {/if}
                                {if (count($this->persons->filter('ROLE_ACTOR')))}
                                    <div class="one_features">Актеры: {foreach from=$this->persons->filter('ROLE_ACTOR') item='person' name='per'}<b>{$person->name}{if $smarty.foreach.per.last}{else},{/if}</b> {/foreach}</div>
                                {/if}
                                {if (count($this->persons->filter('ROLE_OPERATOR')))}
                                    <div class="one_features">Оператор: {foreach from=$this->persons->filter('ROLE_OPERATOR') item='person' name='per'}<b>{$person->name}{if $smarty.foreach.per.last}{else},{/if}</b> {/foreach}</div>
                                {/if}
                                {if (count($this->persons->filter('ROLE_SCENARIST')))}
                                    <div class="one_features">Автор сценария: {foreach from=$this->persons->filter('ROLE_SCENARIST') item='person' name='per'}<b>{$person->name}{if $smarty.foreach.per.last}{else},{/if}</b> {/foreach}</div>
                                {/if}
                                {if (count($this->persons->filter('ROLE_COMPOSITOR')))}
                                    <div class="one_features">Композитор: {foreach from=$this->persons->filter('ROLE_COMPOSITOR') item='person' name='per'}<b>{$person->name}{if $smarty.foreach.per.last}{else},{/if}</b> {/foreach}</div>
                                {/if}
                                {if (count($this->persons->filter('ROLE_PRODUCER')))}
                                    <div class="one_features">Продюссер: {foreach from=$this->persons->filter('ROLE_PRODUCER') item='person' name='per'}<b>{$person->name}{if $smarty.foreach.per.last}{else},{/if}</b> {/foreach}</div>
                                {/if}
            ...
            jQuery(function () {
                ...
                function setup(season_id, serie_id) {
                    ...
                    jQuery('#film_pay_start').html(
                            ['<i class="mdi-play mdi"></i> ', serie.name, 'Cерия (', cost, " руб.)"].join('')
                            );
                ...
                function on_serie_access_fail(ci) {
                    ...
                    if (cost != '0') {
                        jQuery('#film_pay_start').show().html(
                                ['<i class="mdi-play mdi"></i> ', serie_num, ' серия (', cost, " руб.)"].join('')
                                );

                        return this;
                    } else {
                        jQuery('#film_pay_start').show().html(
                                ['<i class="mdi-play mdi"></i> ', serie_num, " серия (бесплатно)"].join('')
                                );

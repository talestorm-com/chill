{if 0==1}<script>{/if}{literal}
    (function () {
        ...
        var TEMPLATES = {/literal}{$this->create_front_templates('Fmp')}{literal};
        var markers = {};
        if (container) {
            ...
            if (show_markers && markers_url) {
                ...
                function on_click(e) {
                    ...
                    if (alias && U.isObject(markers[alias]) && U.isObject(markers[alias]._xm_props)) {
                        ...
                        markers[alias]._xm_props.xob.has_default_image = U.NEString(markers[alias]._xm_props.xob.default_image, null) ? true : false;
                        ...
                        if (event_id) {
                            var event_param = {
                                data: U.safeObject(markers[alias]._xm_props.xob),
                                all_markers: markers,
                                command: "select_icon"
                            };
                            EFO.Events.GEM().run(event_id, event_param);
                        }
                    }
                }
                ...
                jQuery(document.getElementById(container)).on('click', '[data-command]', function (e) {
                    var p = jQuery(this);
                    var cmd = U.NEString(p.data('command'), null);
                    if (cmd) {
                        if (event_id) {
                            e.preventDefault ? e.preventDefault() : e.returnValue = false;
                            popup.remove();
                            var event_param = {
                                data: U.safeObject(p.data()),
                                command: cmd
                            };
                            EFO.Events.GEM().run(event_id, event_param);
                        }
